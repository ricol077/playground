...
__ALIGN_BEGIN static uint8_t USBD_HID_CfgDesc[USB_HID_CONFIG_DESC_SIZ] __ALIGN_END =
{
  ...
  0x00,
  64,          /*bInterval: Polling Interval (10 ms) 端点的查询时间 */
  /* 34 */
...
0x95,0x40,//txc//0x16, // REPORT_COUNT (20) 
...
uint8_t usb_packet_len=0;
static uint8_t  USBD_HID_DataOut (void  *pdev, uint8_t epnum)
{
	uint8_t i,j;

	if(USB_Rx_Buffer[0]==HEADER)
	{
		for(j=0;j<HID_OUT_PACKET;j++)	// 清空缓冲 
	  {  
			 	if((USB_Rx_Buffer[j]==TAIL) && (USB_Rx_Buffer[j+1]==TAIL))
					break;
		}
		for(i=0;i<=(j+1);i++)
		{
			 	usb_rcv_buf[i]=USB_Rx_Buffer[i];
				USB_Rx_Buffer[i]=0;
		}
		usb_packet_len=i-1;
		OSMboxPost(M_USB_rsv, &usb_packet_len);
	}
	...
	/* Prepare Out endpoint to receive next packet */
	  DCD_EP_PrepareRx(pdev,
		...
		//USBD_HID_SendReport (pdev, buf,HID_INOUT_BYTES);
