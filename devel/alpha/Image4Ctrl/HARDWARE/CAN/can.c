...
u8 CAN_Mode_Init(u8 tsjw,u8 tbs2,u8 tbs1,u16 brp,u8 mode)
{
	...
	CAN->sFilterRegister[0].FR1=0X40000000;//32位ID
	CAN->sFilterRegister[0].FR2=0X40000000;//32位MASK
	CAN->FA1R|=1<<0;		//激活过滤器0
...
u8 CAN_ReceiveFlg=0;
		...
		{
			Can_Rx_Msg(0,&CAN_id,&CAN_ide,&CAN_rtr,&CAN_rx_len,CAN_rxbuf);
			CAN_ReceiveFlg=1;
		}
		//#endif
#endif
...
u8 Can_Send_Msg_by_index(u8* msg,u32 id,u8 len)
{	
	u8 mbox;
	u16 i=0;	  	 						       
    mbox=Can_Tx_Msg(id,0,0,len,msg);
	while((Can_Tx_Staus(mbox)!=0X07)&&(i<0XFFF))i++;//等待发送结束
	if(i>=0XFFF)return 1;							//发送失败?
	return 0;										//发送成功;
}
...
{	
	u8 mbox,j,k,l,TailLen,length,index;
	u32 id=0;
	u8 txbuf[MAX_LIMIT]={0};
	u16 i=0;	
	txbuf[0]=ResCode;
	txbuf[1]=cmd;
	txbuf[2]=TransLen;
	for(j=0;j<TransLen;j++)
		txbuf[j+3]=TxBuffer[j];
	...
	for(j=0;j<k;j++)
	{
		...
		l = (j==(k-1))? TailLen:8;
			...
			mbox=Can_Tx_Msg(id,0,0,l, &txbuf[index]);
		  index+=8;
