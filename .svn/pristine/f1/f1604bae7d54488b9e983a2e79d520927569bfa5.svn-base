...
*/ 
	...
	if((TickLock & 0x1) !=0)
	{
		...
		{
			if(CycleSTS == READY) // check whether is in cycle mode, if it is, not count down
				TempTickLength--;
		}
	...
	if((TickLock & 0x2)!=0) 
	{
		...
		if(Control_2_Dir==POS_DIR)		
		...
		else if(Control_2_Dir==NEG_DIR)
			{
				...
				if((TempDelta_2<TEMP_GAP_SWAP)&&(TempCtrl_Active_2 !=4))  // to swap the current of peltier
				{
...
*/
		...
		Channel_2_Load(Control_2_Dir,DutyCycle_2);
		*/
...
float Kp[PID_SLOP_SEG_MAX]={20,20};//18;//20;
float Ki[PID_SLOP_SEG_MAX]={0.03,0.03};//0.03;
float Kd[PID_SLOP_SEG_MAX]={1,1};
float Kl[PID_SLOP_SEG_MAX]={0.3,0.3};
...
int PID_Cal(u8 sensor_index,float temperature)
{
	...
	if(temperature>Ktm[sensor_index])
	 	idx=1;
	///////////////
	//Val=(float)Kp*PID_Err_Rec[sensor_index].Current_Derr+ (float)Ki * PID_Err_Rec[sensor_index].Current_Err;
	//Val=(float)Kp*PID_Err_Rec[sensor_index].Current_Err + (float)Ki * PID_Err_Rec[sensor_index].Inte_Err;
	...
	if(MsgStk !=EMPTY)
	{
		...
		switch (PCR_Cycle_Control.CycleValid)
		{
			...
			case PUMP_RUN:
				...
				break;
			case CYCLE_RUN:			  //think about to reload cycle in msg box handler
									// ISR handler will check cycle, sect, stage, then push msg for cycle done
